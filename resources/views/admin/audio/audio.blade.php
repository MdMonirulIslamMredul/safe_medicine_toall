...
@section('body')
    <div class="row mt-2">
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    <form class="form-horizontal" action="{{ route('audios.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
            ...
            <div class="card-header">
                <table id="config-table" class="table display table-striped border no-wrap">
                    ...
                    @foreach ($audios as $key => $audio)
                        <tr>
                            <td>{{ ++$key }}</td>
                            <td>{{ $audio->updated_at->format('d-M-Y') }}</td>
                            <td>{{ $audio->category->category_name??null }}</td>
                            <td>{{ $audio->subcategory->subcategory_name??null }}</td>
                            <td>{{ $audio->title_en??null }}</td>
                            <td>
                                <audio controls>
                                ...
                                {{-- <img src="{{ asset('audio_files') }}/{{ $audio->audio_file }}" style="height: 100px"> --}}
                            </td>
                            <td>
