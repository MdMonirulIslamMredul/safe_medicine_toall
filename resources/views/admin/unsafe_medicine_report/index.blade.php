...
@section('title') | Medicine Reports @endsection

@section('body')

{{-- ══════════════════════════════════════════════════════════
     STAT COUNTER CARDS  (clickable status filters)
══════════════════════════════════════════════════════════════ --}}
<div class="row mt-3 mb-1 g-3">

    {{-- ALL --}}
    <div class="col-6 col-sm-4 col-lg">
        <a href="{{ route('admin.medicine.reports') }}" class="text-decoration-none">
            <div class="card border-0 shadow-sm h-100"
                 style="{{ !request('status') ? 'border-left:4px solid #343a40;' : '' }}">
                <div class="card-body d-flex align-items-center gap-3 py-3">
                    ...
                    <div>
                        <div style="font-size:1.6rem;font-weight:800;line-height:1;color:#343a40;">{{ $counts['all'] }}</div>
    ...
    {{-- PENDING --}}
    <div class="col-6 col-sm-4 col-lg">
        <a href="{{ route('admin.medicine.reports', ['status'=>'pending']) }}" class="text-decoration-none">
            <div class="card border-0 shadow-sm h-100"
                 style="{{ request('status')=='pending' ? 'border-left:4px solid #f59f00;' : '' }}">
                <div class="card-body d-flex align-items-center gap-3 py-3">
                    ...
                    <div>
                        <div style="font-size:1.6rem;font-weight:800;line-height:1;color:#f59f00;">{{ $counts['pending'] }}</div>
    ...
    {{-- INVESTIGATING --}}
    <div class="col-6 col-sm-4 col-lg">
        <a href="{{ route('admin.medicine.reports', ['status'=>'investigating']) }}" class="text-decoration-none">
            <div class="card border-0 shadow-sm h-100"
                 style="{{ request('status')=='investigating' ? 'border-left:4px solid #0dcaf0;' : '' }}">
                <div class="card-body d-flex align-items-center gap-3 py-3">
                    ...
                    <div>
                        <div style="font-size:1.6rem;font-weight:800;line-height:1;color:#0a9ab5;">{{ $counts['investigating'] }}</div>
    ...
    {{-- RESOLVED --}}
    <div class="col-6 col-sm-4 col-lg">
        <a href="{{ route('admin.medicine.reports', ['status'=>'resolved']) }}" class="text-decoration-none">
            <div class="card border-0 shadow-sm h-100"
                 style="{{ request('status')=='resolved' ? 'border-left:4px solid #198754;' : '' }}">
                <div class="card-body d-flex align-items-center gap-3 py-3">
                    ...
                    <div>
                        <div style="font-size:1.6rem;font-weight:800;line-height:1;color:#198754;">{{ $counts['resolved'] }}</div>
    ...
    {{-- REJECTED --}}
    <div class="col-6 col-sm-4 col-lg">
        <a href="{{ route('admin.medicine.reports', ['status'=>'rejected']) }}" class="text-decoration-none">
            <div class="card border-0 shadow-sm h-100"
                 style="{{ request('status')=='rejected' ? 'border-left:4px solid #dc3545;' : '' }}">
                <div class="card-body d-flex align-items-center gap-3 py-3">
                    ...
                    <div>
                        <div style="font-size:1.6rem;font-weight:800;line-height:1;color:#dc3545;">{{ $counts['rejected'] }}</div>
...
<div class="d-flex align-items-center justify-content-between mt-3 mb-2 flex-wrap" style="gap:8px;">
    <div>
        <h5 class="mb-0 fw-bold">
            ...
            {{ request('status') ? ucfirst(request('status')).' Reports' : 'All Reports' }}
        </h5>
    ...
    @if(request('status'))
        <a href="{{ route('admin.medicine.reports') }}" class="btn btn-sm btn-outline-secondary">
            ...
            <table class="table table-hover align-middle mb-0" style="min-width:820px;">
                ...
                @forelse($reports as $i => $report)
                    ...
                    <tr>
                        ...
                        <td style="padding:12px 16px;">
                            <span style="font-weight:600;">{{ $report->medicine_name }}</span>
                            @if($report->batch_number)
                                <br><small class="text-muted">Batch: {{ $report->batch_number }}</small>
                            @endif
                        </td>

                        <td style="padding:12px 16px;">{{ $report->pharmacy_name }}</td>
                        <td style="padding:12px 16px;">{{ $report->district }}</td>

                        <td style="padding:12px 16px;">
                            @if($report->reporter_name)
                                <span style="font-weight:500;">{{ $report->reporter_name }}</span>
                                @if($report->reporter_phone)
                                    <br><small class="text-muted">{{ $report->reporter_phone }}</small>
                                @endif
                            @else
                        ...
                        <td style="padding:12px 16px;">
                            <span style="font-size:.85rem;">{{ $report->created_at->format('d M Y') }}</span>
                            <br><small class="text-muted">{{ $report->created_at->format('h:i A') }}</small>
                        </td>

                        {{-- View button → dedicated detail page --}}
                        <td style="padding:12px 16px; text-align:center;">
                            <a href="{{ route('admin.medicine.reports.view', $report->id) }}"
                               class="btn btn-sm"
